...
def homo_warp(src_feat, proj_mat, depth_values, return_weights=False):
    """
    src_feat: (B, C, H, W)
    ...
    src_grid_d = R @ ref_grid_d * depth_values.view(B, 1, D * H * W) + T
    ...
    src_grid_d[:, 2:3][negative_depth_mask] = 1000

    if return_weights:
        inverse_depth_src = 1.0 / src_grid_d[:, 2:].reshape(B, 1, H, W)
    ...
    if return_weights:
        return {'warped_feats':warped_src_feat,'inv_depth':inverse_depth_src}
    else:
        return {'warped_feats':warped_src_feat}
