...
class DTUDataset(Dataset):
    ...
    def build_metas(self):
        self.metas = []
        with open(f'datasets/lists/dtu_novelViews/{self.split}.txt') as f:
            ...
            with open(os.path.join(self.root_dir, pair_file)) as f:
                ...
                for _ in range(num_viewpoint):
                    ...
                    for light_idx in light_idxs:
                        ...
                        if self.split == 'train':
                            import random
                            tmp_ = random.sample(src_views, self.n_views)
                            self.metas += [(scan, light_idx, ref_view, tmp_)]
    ...
    def build_proj_mats(self):
        ...
        for vid in range(49): # total 49 view ids
            ...
            proj_mats += [(proj_mat_ls, depth_min,extrinsics)]
    ...
    def __getitem__(self, idx):
        ...
        # Add randomness into the list of source views
        # if self.split == 'train':
        ...
        for i, vid in enumerate(view_ids):
            ...
            proj_mat_ls, depth_min, extrinsics = self.proj_mats[vid]
            ...
            if i == 0:  # reference view
                ...
                ref_proj = proj_mat_ls

                # R_ref = Quaternion(R.from_matrix(extrinsics[:3,:3]).as_quat())
                # T_ref = extrinsics[:3,3]
            else:
                proj_mats += [proj_mat_ls @ ref_proj_inv]
                proj_mats_ref2inputs += [ref_proj @ torch.inverse(proj_mat_ls)]
                # Cycle consistency


                # R_src = Quaternion(R.from_matrix(extrinsics[:3,:3]).as_quat())
                # T_src = extrinsics[:3, 3]
                #
                # distance_angular = Quaternion.distance(R_ref,R_src)
                # distance_translation = distance.euclidean(T_ref,T_src)
                #
                # weight_angular = alpha * np.exp(-distance_angular/pow(sigma_a,2))
                # weight_translation = alpha * np.exp(-distance_translation / 1000)
                # weight = weight_angular * weight_translation
